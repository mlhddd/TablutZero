...
class Net(nn.Module):
    ...
    def __init__(self, num_channels=256, num_res_blocks=7):
        ...
        self.policy_fc = nn.Linear(16 * 9 * 9, 1280)
        # 价值头
...
class PolicyValueNet:
    ...
    def policy_value_fn(self, board):
        self.policy_value_net.eval()
        # 获取合法动作列表
        legal_positions = board.available_actions
        current_state = np.ascontiguousarray(board.current_state().reshape(-1, 5, 9, 9)).astype('float16')
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, lr=0.002):
        ...
        # 价值损失
        mse_loss = nn.MSELoss()
        value_loss = mse_loss(input=value, target=winner_batch)
        # 策略损失
...
if __name__ == '__main__':
    # net = Net().to('cuda')
    # test_data = torch.ones([8, 5, 9, 9]).to('cuda')
    # x_act, x_val = net(test_data)
    # print(x_act.shape)  # 8, 2086
    # print(x_val.shape)  # 8, 1
    net = PolicyValueNet()
    fake_input = torch.randn(1, 5, 9, 9)  # 假设输入是1个批次，5个通道，9x9棋盘

    # 使用 torchviz 生成可视化图
    out = net(fake_input)
    model_graph = make_dot(out, params=dict(list(net.named_parameters()) + [('input', fake_input)]))
    model_graph.render('network_visualization', format='png')  # 保存到文件
    model_graph.view()  # 显示图形